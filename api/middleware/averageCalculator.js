...
const calculateAveragePrice = async (req, res, next) => {
    ...
    try {
        ...
        if (hotel.rooms.length === 0) {
            ...
            console.log(chalk.magenta(`Total price for hotel ${hotelId}: ${totalPrice}, average price: ${hotel.averagePrice}`));
        }

        await hotel.save(); // Save the updated average price in the hotel
        console.log(chalk.green(`Hotel ${hotelId} saved with updated average price: ${hotel.averagePrice}`));
...
const calculateAverageRating = async (req, res, next) => {
    ...
    try {
        ...
        console.log(chalk.magenta(`Calculated average rating for hotel ${hotel._id}: ${hotel.averageRating}`));
    ...
    try {
        ...
        for (const hotel of hotels) {
            ...
            if (hotel.rooms.length === 0) {
                ...
                hotel.averagePrice = parseFloat((totalPrice / hotel.rooms.length).toFixed(1)); // Round to 1 decimal place
                console.log(chalk.magenta(`Hotel ${hotel._id} total price: ${totalPrice}, average price: ${hotel.averagePrice}`));
            }
            ...
            console.log(chalk.magenta(`Hotel ${hotel._id} total ratings: ${totalRating}, total reviews: ${totalReviews}, average rating: ${hotel.averageRating}`));
